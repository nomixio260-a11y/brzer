...
**ヴォルネ三日間** ─ 峠で時間を稼ぎ、橋で敵を止め、奪われた町を取り返す。
...
**継続の令**（戒厳令・情報統制・保安部・増産）は解くまで毎晩効き続ける。
...
戒厳令も情報統制も保安部も粛清も、**恐怖**を生む。
そして恐怖で統治された軍では、部下は悪い報せを上げない。
...
歪むのは**報告だけ**で、真実は歪まない。講評で開く地図には、何が本当に起きていたかが出る。
講評には「貴官が受けていた報告」の一行も出る ─
どれだけ甘い数字を見せられていたかを、そこで初めて知ることになる。
...
どちらでも、その場で失脚する。戦線がどうであろうと、戦争はそこで終わる。
目盛りが 0 になった瞬間に終わるのではなく、低いまま日が過ぎると、ある朝それが起きる。
...
```sh
npm test              # シミュレーション健全性（362項目、ブラウザ不要）
